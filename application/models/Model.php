...
// echo $this->db->last_query();exit;
	...
	public function upload_single_file($fieldName,$renameFile=NULL,$uploadPath=NULL,$allowedTypes=NULL){
		
		$config['upload_path']		=	($uploadPath)?$uploadPath:$this->uploadPath;
		$config['allowed_types']	=	($allowedTypes)?$allowedTypes:$this->allowedTypes;
		$config['max_size']			=	1024*10; // 10 mb
 ...
 //            LEFT JOIN `users` AS `us` ON `or`.`assigned_to` = `us`.`user_id`
	...
	public function product_list(){
		$CI = &get_instance();
		$febric_array = $CI->get_febric_array();
		$request= $_REQUEST;
		$columns = array( 
			0 => 'sku', 
			1 => 'sku_id',
		...
		$count_query = "SELECT count(*) FROM `products`";	//`sku`,`sku_id`,`name`,`febric`,`photo`,`quantity`
		...
		if( !empty($request['search']['value']) ) {
			...
			$where .= "AND ( ";
			$where .="`sku` LIKE '%".$search_value."%' ";
			$where .="OR `sku_id` LIKE '%".$search_value."%' ";
			$where .="OR `name` LIKE '%".$search_value."%' ";
			// if($ss =array_search($search_value,$febric_array)){
			// 	$where .="OR `febric` LIKE '%".$ss."%' ";
			// 	// $where .="OR `febric` LIKE '%".$search_value."%' ";
			// }
			$where .="OR `febric` LIKE '%".$search_value."%' ";
			$where .="OR `quantity` = '".$search_value."'";
			$where .= " )";
		}
